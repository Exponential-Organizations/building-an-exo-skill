...
CLAUDE_SKILL="$REPO_ROOT/dist/building-an-exo-claude.skill"
...
if [[ -z "$NOTES" ]]; then
  NOTES="ExO 3.0 the Organizational Singularity Skill v${VERSION}. Two installable .skill archives: building-an-exo-claude.skill (Claude) and building-an-exo-codex.skill (Codex)."
fi
